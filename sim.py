...
def simNoUncer_interp(p, model, Tstart=0, Astart=0.0, Pstart=0.0, Vstart= -1.0*np.ones((2,2,2,2,2)),cadjust=1.0):
    ...
    #Call the simulator
    epath,ppath,cpath,apath,hpath,pepath,pepath2,pepath3,vpath,evpath,wpath=\
    ...
    return {'wh':epath,'p':ppath,'c':cpath,'A':apath,'h':hpath,'pb':pepath, 'pb2':pepath2,'pb3':pepath3, 'v':vpath,'ev':evpath,'w':wpath}
...
def fast_simulate(Tstart,Astart,Pstart,Vstart,amax,T,N,agrid,pgrid,w,E_bar_now,Pmax,add_points,tw,ts,wls,nwls,δ,q_mini,q,β,γh,σ,
    ...
    for n in prange(N):
        for t in range(Ti,T):  # loop through time periods for a pticular individual
            ...
            mp3=add_points if t>=4 else 1.0
    ...
    return epath,ppath,cpath,apath,hpath,pepath,pepath2,pepath3,vpath,evpath,wpath
