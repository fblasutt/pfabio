        ...
        for i_a in range(Na):  
            for i_w in range(nw):  
                for i_q in range(nq):  
                    ...
                    #valid[i_b,i_a,i_w,i_q] &= m[i_b,i_a,i_w,i_q] > -0.1  
                    #valid[i_b,i_a,i_w,i_q] &= n[i_b,i_a,i_w,i_q] > -0.1  
                    #valid[i_b,i_a,i_w,i_q] &= m[i_b,i_a,i_w,i_q] < pgrid[-1] + 1  
                    #valid[i_b,i_a,i_w,i_q] &= n[i_b,i_a,i_w,i_q] < agrid[-1] + 1  
...
def upperenvelope(out_c,out_n,pmua,out_v,holes,i_a,i_b,tri,i_w,i_q, 
                  ...
                  Na,Nb,valid,num,w,  
                  γc,γh,ρ,agrid,pgrid,β,r,wt,τ,y_N,E_bar_now,Pmax,δ,q,amin,wls,mp,q_mini,wls_point,ζ, 
                  egm_extrap_add=1,egm_extrap_w=-0.05):  
    ...
    for j in range(2):  
        ...
        for i_n in range(in_low,in_high):         
            ...
            for i_m in range(im_low,im_high):  
                ...
                v_interp=np.log(c_interp)-q[i_q,num,i_w]+w_interp/(1+δ)
            ...
            for i_w in range(nw):  
                for i_q in range(nq):  
                    for j in range(2):  
                        ...
                        if holes[i_n,i_m,i_w,i_q] == 1: continue  
        ...
        for i_n in range(i_n_min,i_n_max):  
            for i_m in range(i_m_min,i_m_max):  
                for i_q in range(nq):  
                    ...
                    if holes[i_n,i_m,i_w,i_q] == 0: # if not hole  
                        continue  
                    ...
                    for i_n_close in range(i_n_close_min,i_n_close_max):  
                        for i_m_close in range(i_m_close_min,i_m_close_max):  
                            for j in range(2):#= is not bc, 1 is bc  
                                ...
                                if holes[i_n_close,i_m_close,i_w,i_q] == 1: # if itself a hole  
                                    continue  
