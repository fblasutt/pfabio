...
# xc=np.array([0.04933594, 0.26269531, 0.43964844, 3.53910352])


# xc=np.array([0.01015314, 0.22011626, 0.39937878, 2.55206662])
...
xl=np.array([-0.15,0.05,0.05,.0001]) 
xu=np.array([0.4, 0.6, 1.0,6.0]) 
...
def q(pt,additional_tests=False): 
    ...
    try: 
        ...
        p.q_gridt = np.linspace(-p.qvar,p.qvar,p.nq)#grid_fat_tails(-p.qvar,p.qvar,p.nzw)# np.linspace(p.qmean-p.qmean*p.qvar,p.qmean+p.qmean*p.qvar,p.nq)#co.dist_gamma(p.qshape,p.qscale,p.nq) 
        ...
        if additional_tests:
            ...
            table=r'\begin{table}[htbp]\centering'+\
                r'\begin{threeparttable}'+\
                    ...
                    r'\bottomrule '+\
                    r'\end{tabular}'+\
                    r'\begin{tablenotes}[flushleft]\small\item \textsc{Notes:} The marginal propensity to consume is computed using the methodology in \cite{golosov2024}.''\\\\'+\
             r'\end{tablenotes}'+\
        ...
        # print("The point is {}".format(np.array([((sh_full-.288)/.288)**2,((sh_part-.2)/.2)**2,((sh_min-.15)/.15)**2,((eff_earn-1.158))**2]).sum()))
        # return [((sh_full-.288)/.288),((sh_part-.2)/.2),((sh_min-.15)/.15),((eff_earn-1.158))]  


        # print("The point is {}".format(np.array([((sh_full-.288)/.288)**2,((sh_part-.2)/.2)**2,((sh_min-.2)/.2)**2,((eff_earn-1.158))**2]).sum()))
        # return [((sh_full-.288)/.288),((sh_part-.2)/.2),((sh_min-.2)/.2),((eff_earn-1.158))]  
        ...
        print("The point is {}".format(np.array([((sh_full-0.259095)/0.259095)**2,((sh_part-0.180)/0.180)**2,((sh_min-0.236833)/0.236833)**2,((eff_h-3.04)/3.04)**2]).sum()))
        return [((sh_full-0.259095)/0.259095),((sh_part-0.180)/0.180),((sh_min-0.236833)/0.236833),((eff_h-3.04)/3.04)]  
...
if __name__ == '__main__': 
    ...
    local_search_options = {  "algorithm"    : "dfols", # local search algorithm 
                              ...
                              "num_restarts" : 10,      # how many local searches to do 
                              "shrink_after" : 10,       # after the first [shrink_after] restarts we begin searching 
    ...
    # res=dfols.solve(q, xc, rhobeg = 0.2, rhoend=1e-5, maxfun=150, bounds=(xl,xu), 
