...
def q(pt):
    ...
    sh05=np.mean(SB['h'][3:11,:]==0.5)
    eff=np.mean(SP['h'][3:11,:])-np.mean(SB['h'][3:11,:])
    ...
    return ((shpo-0.64)/0.64)**2+((sh05-0.33)/0.33)**2+((eff-0.099)/0.099)**2#ans
...
xl=np.array([0.06,0.4,0.001])
xu=np.array([0.12,0.8,0.01])

# #Optimization below
# res=dfols.solve(q, xc, rhobeg = 0.1, rhoend=1e-8, maxfun=600, bounds=(xl,xu),
...
#The point is [0.09567203 0.55005277 0.0077236 ], the moments are 0.6376375, 0.3382125, 0.09378124999999998
  
res = minimize(q,xc,method='BFGS' ,bounds=list(zip(list(xl), list(xu))))
