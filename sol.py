...
def solveEulerEquation(p,model='baseline'):
    ...
    policyA1,policyh,policyC,V,policyp,pmutil,whic= np.zeros((7,p.T,4,p.numPtsA, p.numPtsP,p.nw))-np.inf
    holes=np.ones((p.T,4,p.numPtsA, p.numPtsP,p.nw))
...
def solveEulerEquation1(policyA1, policyh, policyC, policyp,V,pmutil,whic,holes,reform,p):
    ...
    ce,he,pe,ae=np.zeros((4,4,numPtsA, numPtsP,nw))
    V1,c1,pmu=np.zeros((3,numPtsA, numPtsP,nw))
    ...
    policyA1[T-1,3,:,:,:] = np.zeros((numPtsA, numPtsP,nw))   # optimal savings
    policyh[T-1,3,:,:,:] = np.zeros((numPtsA, numPtsP,nw))    # optimal earnings
    ...
    for t in range(T-2,-1,-1):
        ...
        if (t+1<=R):    
            ...
            ae[1,...]=(cgrid_box - wt*(1-τ)*he[1,...] - y_N)/(1+r)#Savings
            ce[1,...]=cgrid_box.copy()
            
            #Constrained hours and assets...
            he[2,...]=np.zeros((numPtsA, numPtsP,nw))
            pe[2,...]=pgrid_box.copy()      #Pens. points
            ae[2,...]=(cgrid_box - y_N)/(1+r)#Savings
            ce[2,...]=cgrid_box.copy()
            
            #Constrained (hours) -> Euler equation already writte above
            ce[3,...]=ce[1,...].copy()
            he[3,...]=np.zeros((numPtsA, numPtsP,nw))
            ...
            he[3,...]=np.zeros((numPtsA, numPtsP,nw))                    #Hours
            pe[3,...]=pgrid_box.copy()                                   #Pens. points 
        ...
        if (t+1<=R):     
            ...
            for i in range(4):
                ...
                upperenvelop.compute(policyC[t,i,...],policyh[t,i,...],V[t,i,...],holes[t,i,...],
                        ...
                        V1,
                        γc,maxHours,γh,ρ,agrid,pgrid,β,r,wt,τ,y_N,E_bar_now,δ,pen) 
        ...
        Pmua=np.zeros((4,numPtsA, numPtsP,nw))
        for pp in range(4):
