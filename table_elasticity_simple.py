...
Sρ0= sim.simNoUncer_interp(pρ0,Modρ0,Tstart=np.zeros(p.N,dtype=np.int16)+beg,Astart=SB['A'],Pstart=SB['p'],izstart=SB['iz'])
...
Sρ= sim.simNoUncer_interp(pρ,Modρ,Tstart=np.zeros(p.N,dtype=np.int16)+beg,Astart=SB['A'],Pstart=SB['p'],izstart=SB['iz'])

#2) Increase in gross wages
pτ = co.setup();pτ.w[beg:end]=(1.0+increase)*p.w[beg:end]
Modτ = sol.solveEulerEquation(pτ,model='baseline')
Sτ= sim.simNoUncer_interp(pτ,Modτ,Tstart=np.zeros(p.N,dtype=np.int16)+beg,Astart=SB['A'],Pstart=SB['p'],izstart=SB['iz'])
...
SM= sim.simNoUncer_interp(pM,ModM,Tstart=np.zeros(p.N,dtype=np.int16)+beg,Astart=SB['A'],Pstart=SB['p'],izstart=SB['iz'])

# 2) Hicks elasticity
pH = co.setup();pH.w[end]=(1.0+increase_small)*p.w[end]
ModH = sol.solveEulerEquation(pH,model='baseline')
SH= sim.simNoUncer_interp(pH,ModH,Tstart=np.zeros(p.N,dtype=np.int16)+end,Astart=SB['A'],Pstart=SB['p'],izstart=SB['iz'])

# 3) Frisch elasticity
pF = co.setup();pF.w[end]=(1.0+increase_small)*p.w[end]
ModF = sol.solveEulerEquation(pF,model='baseline')
SF= sim.simNoUncer_interp(pF,ModF,Tstart=np.zeros(p.N,dtype=np.int16)+beg,Astart=SB['A'],Pstart=SB['p'],izstart=SB['iz'])
